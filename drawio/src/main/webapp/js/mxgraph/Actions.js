...
// Added EFE 20170718
	this.addAction('quit', function() 
	{ 	// select tab "Diagram"
		$('a[title="Diagram"]')[0].click();
		// reload the page
		location.reload();
	}, null, null, Editor.ctrlKey + '+Q').isEnabled = isGraphEnabled;
	// Copied and adapted from diagramly/Menu.js (replace editorUi by ui)
	this.addAction('shapes...', function()
	{
//		if (mxClient.IS_CHROMEAPP /*|| !editorUi.isOffline()*/)
		{
			ui.showDialog(new MoreShapesDialog(ui, true).container, 640, /*(isLocalStorage) ?
				((mxClient.IS_IOS) ? 480 : 460) :*/ 440, true, true);
		}
//		else
//		{
//			ui.showDialog(new MoreShapesDialog(ui, false).container, 360, /*(isLocalStorage) ?
...
//					break;
				if (!shape.state.cell.constraints)
					shape.state.cell.constraints = mxUtils.clone(shape.constraints);
				var cellconstraints = shape.state.cell.constraints;
				// select current constraints of East border
				var eastconstraints = cellconstraints
				.filter(function(element)
						{	return (element.point.x == 0);
						})
				.sort(function(a,b) 
						{return (a.point.y > b.point.y) ? 1 : ((a.point.y < b.point.y) ? -1 : 0);
						});
				for (var iconstraint = 1 ; iconstraint < eastconstraints.length; iconstraint++)
				{
					if (typeof(eastconstraints[iconstraint]) != 'function')
					{
						// add 1 constraint between both existing ones
						var i = cellconstraints.push(mxUtils.clone(eastconstraints[iconstraint]));
						cellconstraints[i-1].point.y = (eastconstraints[iconstraint-1].point.y + eastconstraints[iconstraint].point.y)/2;
					}
				}
				// add 1 constraint before start
				var i = cellconstraints.push(mxUtils.clone(eastconstraints[0]));
				cellconstraints[i-1].point.y = eastconstraints[0].point.y/2;
				// add 1 constraint after the end
				var i = cellconstraints.push(mxUtils.clone(eastconstraints[0]));
				cellconstraints[i-1].point.y = (eastconstraints[eastconstraints.length-1].point.y+1)/2;
				// select current constraints of West border
				var westconstraints = cellconstraints
				.filter(function(element)
						{	return (element.point.x == 1);
						})
				.sort(function(a,b) 
						{return (a.point.y > b.point.y) ? 1 : ((a.point.y < b.point.y) ? -1 : 0);
						});
				for (var iconstraint = 1 ; iconstraint < westconstraints.length; iconstraint++)
				{
					if (typeof(westconstraints[iconstraint]) != 'function')
					{
						// add 1 constraint between both existing ones
						var i = cellconstraints.push(mxUtils.clone(westconstraints[iconstraint]));
						cellconstraints[i-1].point.y = (westconstraints[iconstraint-1].point.y + westconstraints[iconstraint].point.y)/2;
					}
				}
				// add 1 constraint before start
				var i = cellconstraints.push(mxUtils.clone(westconstraints[0]));
				cellconstraints[i-1].point.y = westconstraints[0].point.y/2;
				// add 1 constraint after the end
				var i = cellconstraints.push(mxUtils.clone(westconstraints[0]));
				cellconstraints[i-1].point.y = (westconstraints[westconstraints.length-1].point.y+1)/2;
				editor.modified = true;
			}
		}
	}, null, null, Editor.ctrlKey + '+Shift+L');
	this.addAction('connectorstb', function()
	{
		var selectedCells = graph.getSelectionCells();
		for (var icell in selectedCells)
		{
...
//					break;
				if (!shape.state.cell.constraints)
					shape.state.cell.constraints = mxUtils.clone(shape.constraints);
				var cellconstraints = shape.state.cell.constraints;
				// select current constraints of North border
				var northconstraints = cellconstraints
				.filter(function(element)
						{	return (element.point.y == 0);
						})
				.sort(function(a,b) 
						{return (a.point.x > b.point.x) ? 1 : ((a.point.x < b.point.x) ? -1 : 0);
						});
				for (var iconstraint = 1 ; iconstraint < northconstraints.length; iconstraint++)
				{
					if (typeof(northconstraints[iconstraint]) != 'function')
					{
						// add 1 constraint between both existing ones
						var i = cellconstraints.push(mxUtils.clone(northconstraints[iconstraint]));
						cellconstraints[i-1].point.x = (northconstraints[iconstraint-1].point.x + northconstraints[iconstraint].point.x)/2;
					}
				}
				// add 1 constraint before start
				var i = cellconstraints.push(mxUtils.clone(northconstraints[0]));
				cellconstraints[i-1].point.x = northconstraints[0].point.x/2;
				// add 1 constraint after the end
				var i = cellconstraints.push(mxUtils.clone(northconstraints[0]));
				cellconstraints[i-1].point.x = (northconstraints[northconstraints.length-1].point.x+1)/2;
				// select current constraints of South border
				var southconstraints = cellconstraints
				.filter(function(element)
						{	return (element.point.y == 1);
						})
				.sort(function(a,b) 
						{return (a.point.x > b.point.x) ? 1 : ((a.point.x < b.point.x) ? -1 : 0);
						});
				for (var iconstraint = 1 ; iconstraint < southconstraints.length; iconstraint++)
				{
					if (typeof(southconstraints[iconstraint]) != 'function')
					{
						// add 1 constraint between both existing ones
						var i = cellconstraints.push(mxUtils.clone(southconstraints[iconstraint]));
						cellconstraints[i-1].point.x = (southconstraints[iconstraint-1].point.x + southconstraints[iconstraint].point.x)/2;
					}
				}
				// add 1 constraint before start
				var i = cellconstraints.push(mxUtils.clone(southconstraints[0]));
				cellconstraints[i-1].point.x = southconstraints[0].point.x/2;
				// add 1 constraint after the end
				var i = cellconstraints.push(mxUtils.clone(southconstraints[0]));
				cellconstraints[i-1].point.x = (southconstraints[southconstraints.length-1].point.x+1)/2;
				editor.modified = true;
			}
		}
	}, null, null, Editor.ctrlKey + '+Shift+T');
	this.addAction('connectorsreset', function()
	{
		var selectedCells = graph.getSelectionCells();
		for (var icell in selectedCells)
		{
			if (typeof(selectedCells[icell]) == 'object')
			{
				...
				&& shape.state 
				&& shape.state.cell
				&& shape.state.cell.constraints)
				{
					delete shape.state.cell.constraints;
					editor.modified = true;
				}
			}
		}
